...
def reactor_efficiency(voltage, current, theoretical_max_power):
    ...
    percent_power = (generated_power / theoretical_max_power) * 100

    if percent_power >= 80:
        return 'green'
    elif 60 <= percent_power < 80:
        return 'orange'
    elif 30 <= percent_power < 60:
        return 'red'
    else:
        return 'black'
...
def fail_safe(temperature, neutrons_produced_per_second, threshold):
    ...
    if production_rate < lower_bound:
        return 'LOW'
    elif lower_bound <= production_rate <= upper_bound:
        return 'NORMAL'
    else:
        return 'DANGER'
