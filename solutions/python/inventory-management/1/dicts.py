...
def create_inventory(items):
    ...
    items_noduplicates = set(items) # inventory keywords
    inventory_ready = {} 

    for tool in items_noduplicates:
...
def add_items(inventory, items):
    ...
    for tool in items_inventory.keys():

        # update - in both invenotries
        if (tool in inventory) and (tool in items_inventory):
            inventory[tool] += items_inventory[tool]
            
        # update - nonexisting element
...
def decrement_items(inventory, items):
    ...
    return inventory  # Prevent adding new items
